	...
	public void configure() throws Exception {
		...
		from("timer:hello?period=30s").process(new Processor() {
			...
			public void process(Exchange exchange) throws Exception {
				...
				lifOfGroup.add("(cn=scientists)");
				//lifOfGroup.add("(ou=mathematicians)");
				exchange.getIn().setBody(lifOfGroup); // working

			}
		}).split(body()).log("processing group ${body}").to("ldap:ldapserver?base=dc=example,dc=com&scope=subtree").log("group output body ${body} ").process(new Processor() {
			...
			public void process(Exchange exchange) throws Exception {
				...
				for (int i = 0; i < attributes.get("member").size(); i++) {
					String uniqueMember = (String) attributes.get("member").get(i);
		...
		}).split(body()).parallelProcessing(true).to("direct:ldapresponse").to("file:data/output/?fileName=$simple{date:now:yyyyMMdd}/ldap.csv&fileExist=Append&charset=utf-8&Noop=false&move=done&readLockTimeout=1200");

		/*
		 * from("direct:fetchUidfromLdap").process(new Processor() {
		 * 
		 * @Override public void process(Exchange exchange) throws Exception { // TODO
		 * Auto-generated method stub String uid =
		 * exchange.getIn().getBody(String.class); String s[] = uid.split(","); String
		 * string1[] = s[0].split("="); // System.out.println("string1 " + string1[1]);
		 * exchange.getIn().setBody("uid=" + string1[1]);
		 * 
		 * } }).to("ldap:ldapserver?base=dc=example,dc=com&scope=subtree")
		 * .to("direct:ldapresponse") .to(
			...
			public void process(Exchange exchange) throws Exception {
				...
				Attributes attributes = searchResult.getAttributes();
				  ...
				  ids.next().toString());
					...
					/*
					 * String uid = (String) attributes.get("firstname").get();
				...
				/*
				 * String cn = (String) attributes.get("cn").get(); if (cn == null) cn = "";
